...
import moe.seikimo.legacy.game.commands.ConditionalRunCommand;

@RequiredArgsConstructor
public final class LegacyController implements MonoBehaviour {
    @Getter private final GamepadEx handle;

    /** The profile to use for adapting gamepad inputs into values. */
    @Getter @Setter private LegacyProfile profile = LegacyProfile.DEFAULT;
     ...
     */
    public LegacyController intake(Button button, Command press) {
     ...
     * @return This instance for method chaining.
     */
    public LegacyController intake(Button button, Command press, Command release) {
     ...
     * @return This instance for method chaining.
     */
    public LegacyController launch(Button button, Command press, Command release) {
        this.button(button)
    ...
    public void preUpdate() {
        ...
        if (this.handle.isDown(Button.B)) {
            this.profile = LegacyProfile.FLYSKY;
        } else if (this.handle.isDown(Button.A)) {
            this.profile = LegacyProfile.DEFAULT;
        }
    }
